...
namespace NETMapnik
{
	ImageView::ImageView(Image^ image, unsigned x, unsigned y, unsigned w, unsigned h)
	{
		...
		_img_view = new image_view_ptr(std::make_shared<mapnik::image_view<mapnik::image_data_32>>(p->get_view(x, y, w, h)));
	}
	...
	System::Boolean ImageView::IsSolid()
	{
		...
		{
			mapnik::image_view<mapnik::image_data_32>::pixel_type const* first_row = (*_img_view)->getRow(0);
			mapnik::image_view<mapnik::image_data_32>::pixel_type const first_pixel = first_row[0];
			for (unsigned y = 0; y < (*_img_view)->height(); ++y)
			{
				mapnik::image_view<mapnik::image_data_32>::pixel_type const * row = (*_img_view)->getRow(y);
